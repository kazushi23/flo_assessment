	...
	for {
		...
		case "300":
			if currentNMI != "" || currentInterval != 0 {
				p.handle300ToChannel(fields, currentNMI, currentInterval, filePath, recordCount, fps, out)
			}
		case "900":
			return nil
		}
...
// 			return nil
// 		default:
// 			if recordIndicator != "100" && recordIndicator != "500" {
// 				log.Logger.Warn("unknown record type", zap.Int("line", recordCount), zap.Strings("fields", fields))
// 				fps.addErrorRowCSV(fields, fmt.Errorf("unknown record type: %s", recordIndicator))
// 			}
// 		}
// 	}
