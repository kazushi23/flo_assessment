...
func GenerateMalformNEM12CSV(fileName string, intervalMinutes int, numDays int, numRegisters int) error {
	f, err := os.Create(fileName)
	if err != nil {
		return err
	}
	...
	for day := range numDays {
		...
		for reg := 1; reg <= numRegisters; reg++ {
			...
			values := make([]string, intervals)
			for i := range intervals {
				if rand.Float32() < 0.05 { // 5% chance of malformed value
					values[i] = "ERROR" // non-numeric
				} else {
					values[i] = fmt.Sprintf("%.2f", rand.Float64()*600)
				}
			}
			...
			// 500 - Optional B2B Record
			if rand.Float32() < 0.3 { // ~30% chance to add
				if rand.Float32() < 0.1 { // 10% chance to make malformed
	...
	fmt.Printf("Generated Malformed NEM12 CSV: %s\n", fileName)
	return nil
}
